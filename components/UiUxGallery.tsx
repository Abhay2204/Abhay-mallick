...
export default function UiUxGallery() {
  ...
  useEffect(() => {
    ...
    // Set initial state — panels fully covering the section
    gsap.set(panels, { yPercent: 0 });

    const ctx = gsap.context(() => {
      // Curtain reveal: stagger each panel sliding UP to -100%
      gsap.to(panels, {
        yPercent: -100,
        duration: 1.1,
        ease: 'power3.inOut',
        stagger: {
          amount: 0.5,        // total stagger spread across all 5 panels
          from: 'start',      // left-to-right curtain open
        },
        scrollTrigger: {
          trigger: section,
          start: 'top 80%',   // fire when section top is 80% down the viewport
          toggleActions: 'play none none none', // play once, never reverse
        },
        onComplete: () => {
          // Hide curtain entirely after animation so it doesn't block interaction
    ...
    <section ref={sectionRef} className="w-full bg-graphite text-white relative py-32 overflow-hidden">
      ...
      <SwissGrid className="relative z-10">
        {galleryItems.map((item) => {
          if (item.type === 'text') {
            return (
              <div key={item.id} className={`gallery-item ${item.gridClass} flex flex-col gap-8`} data-speed={item.speed}>
          ...
          if (item.type === 'local-video') {
            return (
              <div key={item.id} className={`gallery-item ${item.gridClass} ${item.aspect} relative overflow-hidden group`} data-speed={item.speed}>
          ...
          if (item.type === 'youtube') {
            return (
              <div key={item.id} className={`gallery-item ${item.gridClass} ${item.aspect} relative overflow-hidden group bg-black/20`} data-speed={item.speed}>
                <div className="absolute inset-0 w-full h-full pointer-events-none overflow-hidden">
                  <iframe
                    ...
                    className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[150%] h-[150%] object-cover scale-100 transition-transform duration-700 opacity-90 group-hover:opacity-100 group-hover:scale-[1.05]"
                    frameBorder="0"
                ...
                {/* Hover Overlay */}
                {item.href ? (
                  <a href={item.href} target="_blank" rel="noopener noreferrer" className="absolute inset-0 bg-graphite/0 group-hover:bg-graphite/40 transition-colors duration-500 flex items-center justify-center cursor-pointer">
                    <div className="opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-y-4 group-hover:translate-y-0 font-mono text-white text-sm tracking-widest uppercase border border-white/50 px-6 py-3 rounded-full backdrop-blur-md bg-white/10 hover:bg-white/20 hover:scale-105">
                      View Project
                    </div>
                  </a>
                ) : (
                  <div className="absolute inset-0 bg-graphite/0 group-hover:bg-graphite/40 transition-colors duration-500 flex items-center justify-center pointer-events-none">
                    <div className="opacity-0 group-hover:opacity-100 transition-all duration-500 transform translate-y-4 group-hover:translate-y-0 font-mono text-white/90 text-sm tracking-widest uppercase border border-white/30 px-6 py-3 rounded-full backdrop-blur-md bg-black/20">
                      {item.title}
                    </div>
                  </div>
                )}
