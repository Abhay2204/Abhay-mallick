...
export default function HeroSection() {
  ...
  useEffect(() => {
    ...
    const str2 = "I am Abhay Mallick, FREELANCER, FULL STACK DEVELOPER.";
    ...
    const ctxGsap = gsap.context(() => {
      const tl = gsap.timeline({
        scrollTrigger: {
          ...
          start: 'top top',
          end: '+=300%', // Reduced scroll distance to escape the frame sooner
          pin: true,
          scrub: 1,
        }
      });
      ...
      tl.to(frameObj, {
        ...
        snap: 'index',
        ease: 'none',
        duration: 1, // Forces canvas images to distribute across the ENTIRE scroll pin
        onUpdate: () => renderFrame(frameObj.index)
      }, 0);

      // ----- Typewriter Seq -----
      // Total frames = 208
      // Frame 61 is at 61 / 207 = 0.294 progress. 
      // We will stretch the typing to end right before 0.294.

      // Type String 1
      tl.to(typedObj, {
        t1: str1.length,
        duration: 0.12,  // String 1 types for 12% of scroll
        ease: 'none',
        onUpdate: () => {
      ...
      tl.to(typedObj, {
        t2: str2.length,
        duration: 0.16, // Starts at 0.13, ends at 0.29 (exactly frame 60-61)
        ease: 'none',
        onStart: () => {
      ...
      }, 0.13); // Stagger gap so there's a tiny pause after string 1
      ...
      tl.to(typedObj, {
        ...
        duration: 0.05,
        ease: 'none',
        onStart: () => { // bring caret 1 back as String 1 deletes
             if (caret1Ref.current) caret1Ref.current.style.opacity = '1';
             if (caret2Ref.current) caret2Ref.current.style.opacity = '0';
        },
        onReverseComplete: () => {
      ...
      // Hide the entire sequence 1 container after it's done deleting so the caret doesn't linger
      tl.to(seq1ContainerRef.current, { opacity: 0, duration: 0.01 }, 0.42);

      // ----- Seq 2 Left/Right Reveal -----
      tl.fromTo(seq2LeftRef.current,
        { opacity: 0, x: -50 },
        { opacity: 1, x: 0, duration: 0.12, ease: 'power2.out' },
        0.47 // Starts revealing shortly after deletion
      );
      ...
      tl.to({}, { duration: 0.15 }, 0.65); // Shorter spacer so that we don't artificially extend the timeline past the images.
    }, containerRef);
